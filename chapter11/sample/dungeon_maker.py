...
def make_dungeon(): # ダンジョンの自動生成
    ...
    # 迷路からダンジョンを作る
    #全体を壁にする（迷路は一旦関係なし。新しく27x33のマスを作る）
    for y in range(DUNGEON_H):      # DUNGEON_H = 27
        for x in range(DUNGEON_W):  # DUNGEON_W = 33
            dungeon[y][x] = 9
    #部屋と通路の配置
    for y in range(1, 8):      # 1~8 7回繰り返す
        for x in range(1, 10):  # 1~10 9回繰り返す
            dx = x*3+1
            dy = y*3+1
            if maze[y][x] == 0:
                if random.randint(0, 99) < 20: # 部屋を作る
                    for ry in range(-1, 2):      # -1~2 = -1,0,1の3回繰り返し
                        for rx in range(-1, 2):  # 同上
